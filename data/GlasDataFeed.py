...
Same as WarwickDataFeed but using a different dataset folder structure.
...
|---- <anno>/<db>_<id>_anno.bmp (Annotation masks or labels, anno in {'anno', 'bb', ...)
|---- <anno>/<db>_<id>_anno_+5.bmp (Annotation masks or labels with dilation by disk(5) for Label Augmentation)
|---- <anno>/<db>_<id>_anno-5.bmp (Annotation masks or labels with erosion by disk(5) for Label Augmentation)
'''
...
class WarwickDataFeed(GenericDataFeed):
    ...
    def __init__(self, params, db, generator=None):
        ...
        self.files_Y = [os.path.join(self.directory, "%s/%s_%d_anno.bmp"%(self.annotations, db, i+1)) for i in range(nPerDb[db])]
        # Label augmentation
        if( self.tda ):
            self.files_Yplus = [os.path.join(self.directory, "%s/%s_%d_anno+5.bmp"%(self.annotations, db, i+1)) for i in range(nPerDb[db])]
            self.files_Yminus = [os.path.join(self.directory, "%s/%s_%d_anno-5.bmp"%(self.annotations, db, i+1)) for i in range(nPerDb[db])]
